...
const form = useForm({
  ...
  image: null,
  imagePreview: props.category.image ? `/storage/${props.category.image}` : null
});

// Generate slug from name when name changes
watch(() => form.name, (newName) => {
  if (newName && !form.isDirty('slug')) {
  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Update Category</h1>
      </div>
    </div>
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div class="sm:col-span-2">
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  The display name for your category
                </p>
              </div>
              ...
              <div class="sm:col-span-2">
                ...
                <div class="mt-1 flex items-center gap-4">
                  <div v-if="form.imagePreview" class="flex-shrink-0">
                    <img class="h-24 w-24 rounded-md object-cover border border-gray-200 dark:border-gray-600" :src="form.imagePreview" :alt="`Preview of ${form.name}`">
                  </div>
                      ...
                      <div class="rounded-md border border-gray-300 dark:border-gray-600 px-3 py-2 text-sm shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700">
                        <div class="flex items-center justify-center space-x-2">
                          ...
                          <span class="text-sm font-medium text-gray-700 dark:text-gray-300">
                            {{ form.image ? 'Change image' : form.imagePreview ? 'Update image' : 'Upload image' }}
                          </span>
                        </div>
                      </div>
                      <input 
                ...
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  Recommended size: 500x500px. JPG, PNG or GIF. Leave unchanged to keep current image.
                </p>
              </div>
            </div>
